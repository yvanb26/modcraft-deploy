...
`site/contact.html` envoie ses messages via `POST /contact` sur votre API, qui les relaie par SMTP vers votre boîte mail (`SMTP_*` et `CONTACT_TO` dans `api/.env`, voir `api/.env.example`). Sans ces variables, la route répond juste « formulaire indisponible » et le reste de l'API continue de fonctionner normalement.

Cette route est appelée depuis un navigateur (contrairement au reste de l'API, appelé par le launcher) : ajoutez l'adresse de votre site à `CORS_ORIGINS` si vous utilisez ce formulaire. Deux protections anti-spam basiques sont en place : un captcha mathématique (`GET /contact/captcha`, réponse à usage unique côté serveur) et un champ piège invisible.
